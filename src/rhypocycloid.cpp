...
bool rhypocycloid::initialCalculations(){
...
void rhypocycloid::startMessage(){
  printData("Ratio: ");
  printData(to_string(CirCurRadius / CirTthRadius));
  printData(" Calculating rhypocycloid...\n");
}

void rhypocycloid::newToothMessage(){
  printData("tooth_" + to_string(CurrentTooth + 1) + "\n");
}

void rhypocycloid::oneToothCalculations(){
...
void rhypocycloid::sectionInit(int sectionNo){
...
bool rhypocycloid::pointInMaterial(const coordinates &point){
  return point.x * point.x + point.y * point.y < MatRadius * MatRadius;
}

bool rhypocycloid::pointInside(const coordinates &point){
  return point.x * point.x + point.y * point.y < CirCurRadius * CirCurRadius;
}

bool rhypocycloid::pointOutsideMiddle(const coordinates &point){
  long double x, y;
...
void rhypocycloid::basePoint(const long double &t){
...
void rhypocycloid::curvePoint(const long double &t){
...
void rhypocycloid::actualizeDist(){
...
void rhypocycloid::generateSection(const long double &str, const long double &end){
  stringstream ss;
  long double t;
  t = str;
  while (t < end) {
...
void rhypocycloid::generateCircle(){
  stringstream ss;
  long double t, tMin;
...
rhypocycloid::rhypocycloid(int materialAddition,
                       int circleDiamCurve,
                       int circleDiamTeeth,
                       int noOfTeeth,
                       int toothDiameter,
